...
def replace_invalid_date(date_str):
      try:
            ...
            date_str = pd.to_datetime(date_str, infer_datetime_format=True)
...
def recommend(dataset: str, uid: str):
  ...
  df['purchase_date'] = pd.to_datetime(df['purchase_date'], format='mixed', infer_datetime_format=True, errors = 'coerce')
